...
if (isServer and isNil "sm_done") then {
				...
				if (_object isKindOf "AllVehicles") then {
					...
					if (_type in DayZ_SafeObjects) then {
						...
						//_booleans=[];
							...
							_pos set [2,0];
							_object setPos _pos; //leave as setPos so we get angle/incline compensation.
						...
						_object setDamage 1;
						diag_log format["OBJ: %1 is not a safe object, Destoryed Damage: %2", _object,_damage];
					};
				};
	...
	//Spawn camps
	dayz_CampsSpawner = [] spawn {
		waitUntil{!isNil "fn_selectRandomLocation"};
		...
		//Spawn camps
	dayz_PlantsSpawner = [] spawn {
		waitUntil{!isNil "fn_selectRandomLocation"};
				
		//Plants
		[1000] call server_plantSpawner;	
	};
	
		//Spawn camps
	dayz_CarePkgSpawner = [] spawn {
		waitUntil{!isNil "fn_selectRandomLocation"};
...
*/
	...
	*/
	...
	dayz_RandomLocation = {
		...
		while {_index < 50} do {
			...
			if ((count _isFlat > 1) AND (NOT isOnRoad _location)) then { _index = 50; };
	...
	*/
	...
	/*
	dayz_testrndspawner = [] spawn {
		while {true} do {
			_positionTest = [getMarkerPos "center",80,150] call fn_selectRandomLocation;
			diag_log (str(_positionTest));
			sleep 1;
		};
	};
	*/
