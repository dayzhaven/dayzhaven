...
_timeleft = 0;
{
	...
	//Player just dissconnected or lost connection.
		if (_playerID == _0) exitWith { _endMission = true; };
	};
	
}forEach dayz_activePlayers;
//Make player wait till ghost timer is up.
if (_endMission) exitwith {
	_remianing = dayz_ghostTimer - _timeleft;
	diag_log format["LOGIN CANCELLED: player: %1 is in ghost mode. Time remianing: %2 before login",_playerObj,_remianing];
};
