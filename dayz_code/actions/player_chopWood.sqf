	...
	if(_distance2d <= 5) then {
		...
		while {_isOk} do {	
			...
			if(_finished) then {
				...
				//"Working",0,[cal,thirst,hunger,temp]
				["Working",0,[250,25,15,0]] call dayz_NutritionSystem;
				
				//ItemHatchetBroken

			if ((_counter == _countOut) || _breaking) exitWith {
				if (_breaking) then {
					cutText ["Whilst chopping the wood the handle breaks.", "PLAIN DOWN"];
				} else {
					cutText ["Finished cutting wood.", "PLAIN DOWN"];
				};
				_isOk = false;
				_proceed = true;
				sleep 1;
