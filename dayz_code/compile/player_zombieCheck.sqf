		...
		if (alive _x) then {
			...
			if (!local _x) then {		
				if (_refObj in _targets) then {
					...
					if (speed _vehicle >= 5) then { _distanceAttack = 3.6; }; //running forward
					if (speed _vehicle <= -5) then { _distanceAttack = 3.3; }; //running backwords
					
					if (_x distance _refObj < _distanceAttack) then {
